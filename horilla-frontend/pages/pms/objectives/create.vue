<template>
  <div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-3xl font-bold text-gray-900">Create Objective</h1>
    </div>
    ...
    <form v-else @submit.prevent="submitForm" class="card space-y-6">
      ...
      <div>
        ...
        <input
          v-model="form.manager_ids_input"
          type="text"
          placeholder="Enter manager IDs separated by commas (e.g., 1,2,3)"
          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <p class="text-xs text-gray-500 mt-1">Enter comma-separated manager IDs</p>
      </div>
...
const submitForm = async () => {
  ...
  try {
    const managerIds = form.value.manager_ids_input
      .split(',')
    ...
    if (objective) {
      ...
      error.value = 'Failed to create objective'
    }
  } catch (err: any) {
    error.value = err.message || 'Failed to create objective'
  } finally {
