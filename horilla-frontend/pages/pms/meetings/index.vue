<template>
  <div class="space-y-6">
    ...
    <!-- Empty State -->
    <div v-else-if="filteredMeetings.length === 0" class="card empty-state">
      <Icon name="heroicons:calendar" class="empty-state-icon" />
      <p class="empty-state-text">No meetings scheduled</p>
    </div>
    ...
    <div v-else class="grid gap-4">
      <div
        ...
        @click="navigateTo(`/pms/meetings/${meeting.id}`)"
      >
        ...
        <div class="flex items-center gap-4 mb-3">
          <span :class="`badge status-${meeting.status.toLowerCase().replace(' ', '-')}`">
            {{ meeting.status }}
          </span>
        ...
        <div class="text-xs text-gray-500 mb-3">
          <div class="flex gap-4">
            <span v-if="meeting.meeting_date">📅 {{ formatDate(meeting.meeting_date) }}</span>
            <span v-if="meeting.meeting_time">🕐 {{ meeting.meeting_time }}</span>
          </div>
        </div>
      </div>
    </div>
...
const { fetchMeetings, deleteMeeting: deleteMt } = useMeetings()
...
const filteredMeetings = computed(() => {
  if (!selectedStatus.value) {
    return pmsStore.meetingsList
  }
  return pmsStore.meetingsList.filter(m => m.status === selectedStatus.value)
})
...
const deleteMeeting = async (id: number) => {
  if (confirm('Are you sure you want to delete this meeting?')) {
    isLoading.value = true
    const success = await deleteMt(id)
    isLoading.value = false
    if (success) {
      await loadMeetings()
    }
