  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-3xl font-bold text-gray-900">Meeting Responses</h1>
    ...
    <div v-else-if="filteredAnswers.length === 0" class="card empty-state">
      ...
      <p class="empty-state-text">No responses submitted yet</p>
      <NuxtLink
        :to="`/pms/meetings/${meetingId}/answer`"
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-gray-600 mt-1">
              ...
              <span class="inline-block px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs">
                {{ answer.response_by || 'Anonymous' }}
              </span>
            </p>
          </div>
          <div class="flex gap-2">
            <NuxtLink
              :to="`/pms/meetings/${meetingId}/answers/${answer.id}/edit`"
    ...
    <!-- Summary Stats -->
    <div v-if="answers.length > 0" class="card bg-purple-50 border border-purple-200">
      <h3 class="font-semibold text-purple-900 mb-3">Summary</h3>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
        <div>
          <p class="text-purple-600 mb-1">Total Responses</p>
          <p class="text-2xl font-bold text-purple-900">{{ answers.length }}</p>
        </div>
        <div>
          <p class="text-purple-600 mb-1">Questions Answered</p>
          <p class="text-2xl font-bold text-purple-900">
            {{ new Set(answers.map(a => a.question_id)).size }} / {{ totalQuestions }}
          </p>
        </div>
        <div>
          <p class="text-purple-600 mb-1">Last Response</p>
          <p class="text-sm text-purple-900">{{ formatDate(lastUpdated) }}</p>
        </div>
      </div>
    </div>
  </div>
...
const { fetchMeetingAnswers } = useMeetings()
...
const loadAnswers = async () => {
  ...
  error.value = null
  try {
    const fetchedAnswers = await fetchMeetingAnswers(meetingId.value)
...
const deleteAnswer = async (answerId: number) => {
  if (confirm('Are you sure you want to delete this response?')) {
    ...
    try {
      ...
      error.value = err.message || 'Failed to delete response'
    } finally {
      isLoading.value = false
    }
  }
