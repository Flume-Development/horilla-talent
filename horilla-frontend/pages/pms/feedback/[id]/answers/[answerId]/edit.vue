<template>
  <div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-3xl font-bold text-gray-900">Edit Feedback Answer</h1>
    </div>
    ...
    <form v-else-if="answer" @submit.prevent="submitEdit" class="card space-y-6">
      ...
      <div class="flex gap-4 pt-6 border-t">
        ...
        >
          ...
          <span v-else>Update Answer</span>
        </button>
        <NuxtLink
          :to="`/pms/feedback/${feedbackId}/answers`"
...
const loadAnswer = async () => {
  ...
  try {
    ...
    error.value = err.message || 'Failed to load answer'
  } finally {
...
const submitEdit = async () => {
  if (!editedAnswer.value) {
    submitError.value = 'Please provide an answer'
    return
  }
  ...
  try {
    ...
    // const success = await updateFeedbackAnswer(answerId.value, { answer: editedAnswer.value })
    // if (success) {
    await router.push(`/pms/feedback/${feedbackId.value}/answers`)
    // } else {
    //   submitError.value = 'Failed to update answer'
    // }
  } catch (err: any) {
    submitError.value = err.message || 'Failed to update answer'
  } finally {
